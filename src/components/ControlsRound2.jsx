...
const METRIC_DEFINITIONS = {
    readability: 'Measures how easy it is to read and understand the translation. Higher scores indicate text that fits well with the cultural and linguistic norms of the target language.',
    adequacy: 'Evaluates how accurately and completely the translation captures the original meaning, including all key details and nuances. Higher scores indicate more faithful and comprehensive translations.',
    grammatically_correct: 'The percentage of sentences that are fully grammatically correct in the target language. Higher percentages indicate fewer grammatical errors.',
    real_words: 'The percentage of sentences that contain only real words from the target language (no gibberish or words from other languages). Higher percentages indicate better word authenticity.',
    notable_error: 'The percentage of sentences containing critical errors that significantly impact meaning, readability, or quality. Lower percentages are better, as they indicate fewer serious mistakes that distort the original message or context.'
};
...
const Controls = ({
    ...
    const [openProviderMenu, setOpenProviderMenu] = useState(null);
    ...
    const handleRoundChange = (round) => {
        ...
        // round2 is current, no action needed
    };
    ...
    const handleDownloadData = (format) => {
        ...
        if (format === 'json') {
            ...
            link.href = url;
            link.download = `round2-analysis-${reviewer}-${Date.now()}.json`;
            ...
            // CSV export
            const headers = ['language', 'model', 'readability_mean', 'readability_stdDev', 'readability_count',
                'adequacy_mean', 'adequacy_stdDev', 'adequacy_count',
                'grammatically_correct_mean', 'grammatically_correct_stdDev', 'grammatically_correct_count',
                'real_words_mean', 'real_words_stdDev', 'real_words_count',
                'notable_error_mean', 'notable_error_stdDev', 'notable_error_count'];
            const rows = data.flatMap(lang =>
                lang.models.map(model => [
                    lang.language,
                    model.name,
                    model.metrics.readability.mean,
                    model.metrics.readability.stdDev,
                    model.metrics.readability.count,
                    model.metrics.adequacy.mean,
                    model.metrics.adequacy.stdDev,
                    model.metrics.adequacy.count,
                    model.metrics.grammatically_correct.mean,
                    model.metrics.grammatically_correct.stdDev,
                    model.metrics.grammatically_correct.count,
                    model.metrics.real_words.mean,
                    model.metrics.real_words.stdDev,
                    model.metrics.real_words.count,
                    model.metrics.notable_error.mean,
                    model.metrics.notable_error.stdDev,
                    model.metrics.notable_error.count
                ])
            );
            const csvContent = [
                headers.join(','),
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `round2-analysis-${reviewer}-${Date.now()}.csv`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(url);
    ...
    // Metric labels for Round 2
    const metricLabels = {
        readability: 'Readability',
        adequacy: 'Adequacy',
        grammatically_correct: 'Grammatical Correct (%)',
                        ...
                        {showLanguageMenu && (
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    padding: '0.75rem',
                                    fontSize: '0.875rem',
                                    backgroundColor: !selectedLanguage && !selectedLanguageGroup ? 'var(--bg-card-hover)' : 'transparent',
                                    color: 'var(--text-primary)',
                                    border: '1px solid var(--border-color)',
                                    borderRadius: '0.375rem',
                                    marginBottom: '0.5rem',
